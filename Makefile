...
ifeq ($(XCODE),ON)
	xcodebuild -verbose -project $(PROJ) -target GoodbyeBigSlow -configuration Release -arch x86_64 MARKETING_VERSION=$(KEXT_VERSION) PRODUCT_BUNDLE_IDENTIFIER=$(KEXT_ID) MODULE_NAME=$(KEXT_ID) MODULE_VERSION=$(KEXT_VERSION) MACOSX_DEPLOYMENT_TARGET=$(MACOS_VERSION_MIN) CODE_SIGN_IDENTITY="-"
else
